...
function resultOutput(player) {
  if (player == "player") {
    ...
    document.querySelector(".main").style.background =
      "radial-gradient(circle, rgba(0,0,0,1) 70%, rgba(132,252,70,1) 100%)";
  } else {
    document.querySelector(".main").style.transition = ".2s";
    document.querySelector(".main").style.background =
      "radial-gradient(circle, rgba(0,0,0,1) 70%, rgba(246,0,0,1) 100%)";
  }
}
...
function playRound(player, computer) {
  ...
  clearResponce(player, computer);
}
function clearResponce(player, computerChoice) {
  setTimeout(() => {
    document.querySelector(".main").style.background = "";
    document.getElementById(computerChoice).style.border = "";
